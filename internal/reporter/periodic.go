...
// GeneratePeriodicReport 通用周期报表生成器 (支持 "weekly", "monthly", "quarterly", "yearly")
func GeneratePeriodicReport(basePath string, periodType string, targetTime time.Time, jumpURL string) (*PeriodicReportData, error) {
	...
	case "monthly":
		...
		prevEndDate = startDate.Add(-1 * time.Second)
		title = fmt.Sprintf("📈 %d年%d月 财务月度盘点", targetTime.Year(), targetMonthTitle(targetTime))
...
func targetMonthTitle(t time.Time) int {
	return int(t.Month())
}
